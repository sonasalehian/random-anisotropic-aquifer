...
import scienceplots
from rose_diagram_intermediate import plot_rose_diagram, extract_bar_parameters
...
@numpyro.handlers.reparam(
    config={
        ...
        "mu_4": numpyro.infer.reparam.CircularReparam(),
    }
)
def model(y_obs=None):
    ...
    kappa_4 = numpyro.sample("kappa_4", dist.Gamma(20.0, 0.1))
    ...
    mu_4 = numpyro.sample(
        "mu_4", dist.VonMises(loc=jnp.radians(0), concentration=1 / jnp.radians(180) ** 2)
    )
    ...
    vm_4 = dist.VonMises(loc=mu_4, concentration=kappa_4)
    ...
    with numpyro.plate("y_obs", len(y_obs) if y_obs is not None else 1):
        _ = numpyro.sample("y", dist.MixtureGeneral(mix, [vm_1, vm_2, vm_3, vm_4]), obs=y_obs)
...
graph = numpyro.render_model(
    ...
    render_params=True,
    filename="output/intermediate/dag-model.pdf"
)
graph
...
file_name = "output/intermediate/summary.csv"
np.savetxt(file_name, summary, delimiter=",")

# Plot mu_1 and mu_2 distributions
az.plot_trace(data, var_names=["mu_1", "mu_2", "mu_3", "mu_4"])
plt.savefig("output/intermediate/plot_trace_4vm.pdf")
...
np.save("output/intermediate/random_rotation_angle.npy", random_rotation_angles)
...
plt.savefig('output/intermediate/rose_diagram_random_rotation_angle.pdf')
