...
bin_edges_original = np.array([0, 10, 20, 30, 90, 100, 110, 120, 130])
frequencies_original = np.array([22, 22, 7.5, 10, 9, 14, 4, 22, 4])
...
def generate_data_from_histogram(bin_edges, frequencies, num_samples):
    ...
    for i in range(num_samples):
        ...
        bin_index = np.random.choice(len(bin_edges), p=frequencies / np.sum(frequencies))
        # Generate a random value within the chosen bin
        lower_bound, upper_bound = bin_edges[bin_index], bin_edges[bin_index] + 10
...
# Save generated data
np.save("output/intermediate/rose_diagram.npy", generated_data)
...
theta = np.radians(bin_edges_original)
plot_rose_diagram(theta, frequencies_original, gradation=10.)
plt.savefig('output/intermediate/rose_diagram_heilweil.pdf')
plt.close()  # Close the figure

# Plot rose diagram of generated data
generated_data = np.load('output/intermediate/rose_diagram.npy')
...
plt.savefig('output/intermediate/rose_diagram_generated_data_from_rose_diagram.pdf')
