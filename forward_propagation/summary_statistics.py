...
# Number of outputs
n_0 = 0
folder_path = f'output/{random_folder}'
prefix = 'run_'
n_outputs = count_files_starting_with(folder_path, prefix)

# Load time steps
filename_output_ts = f'output/{random_folder}/run_{str(n_0).zfill(4)}/output_ts.npy'
ts = np.load(filename_output_ts)

# Read mesh
filename = f"output/{random_folder}/run_{str(n_0).zfill(4)}/solution.bp"
engine = "BP4"
...
for t in ts:
    ...
    for n in range(n_0, n_outputs):
        u_los.name = "u_n_sub"
        filename = f"output/{random_folder}/run_{str(n).zfill(4)}/solution.bp"
