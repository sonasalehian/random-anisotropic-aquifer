...
def boundary_conditions(parameters, domain, ft, V):
    ...
    bcu_bottom = fem.dirichletbc(
        u_D,
        fem.locate_dofs_topological((V.sub(2), U), fdim, ft.find(bottom_marker)),
        V.sub(2),
    )
    ...
    bcp_bottom = fem.dirichletbc(
        f_h,
        fem.locate_dofs_topological((V.sub(0), Q), fdim, ft.find(bottom_marker)),
        V.sub(0),
    )
    bcp_top = fem.dirichletbc(
        f_h,
        fem.locate_dofs_topological((V.sub(0), Q), fdim, ft.find(top_marker)),
        V.sub(0),
    )
    bcp_sidesx = fem.dirichletbc(
        f_h,
        fem.locate_dofs_topological((V.sub(0), Q), fdim, ft.find(sidesx_marker)),
        V.sub(0),
    )
    bcp_sidesy = fem.dirichletbc(
        f_h,
        fem.locate_dofs_topological((V.sub(0), Q), fdim, ft.find(sidesy_marker)),
        V.sub(0),
    )
    bcp_drywell = fem.dirichletbc(
        f_h,
        fem.locate_dofs_topological((V.sub(0), Q), fdim, ft.find(drywell_marker)),
        V.sub(0),
    )
    bcp_pumpingwell = fem.dirichletbc(
        G_h,
        fem.locate_dofs_topological((V.sub(0), Q), fdim, ft.find(pumpingwell_marker)),
        V.sub(0),
    )
    ...
    for tag in material_tags:
        ...
        if tag == 1:
            ...
            res = np.linalg.inv(
                np.array(
                    [
                        [k_x / mu_f, k_xy / mu_f, 0],
                        [k_yx / mu_f, k_y / mu_f, 0],
                        [0, 0, k_z / mu_f],
                    ]
                )
            )
...
def solve(parameters):
    ...
    [f_p, invkappa, S_e, lmbda, G, alpha] = equation_parameters(parameters, domain, mt)
    print_root("Done.")
    ...
    # Weak Forms #
    a = (
        ufl.inner(S_e * p, p_t) * dx
        + ufl.inner(alpha * ufl.div(u), p_t) * dx
    ...
    LOS = np.array(
        [
            [
                np.sin(incidence_angle) * np.cos(satellite_heading),
                np.sin(incidence_angle) * np.sin(satellite_heading),
    ...
    Q0 = dolfinx.fem.functionspace(
        domain,
        basix.ufl.element("Discontinuous Lagrange", "tetrahedron", 1, shape=(3,)),
    )
    qh_Q0 = fem.Function(Q0, dtype=PETSc.ScalarType)
    qh_Q0.interpolate(q_n)

    P0 = dolfinx.fem.functionspace(
        domain, basix.ufl.element("Discontinuous Lagrange", "tetrahedron", 1)
    )
    ...
    for i in range(num_steps):
        ...
        print_root("Started linear solve...")
